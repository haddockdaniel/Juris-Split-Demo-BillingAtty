...
//            listBoxCompanies.SelectedIndexChanged += listBoxCompanies_SelectedIndexChanged;
        ...
        private void runSQLqueries(string billAtty, string firstAtty, string origDB)
        {
            ...
            try
            {
                ...
                errorTable = "FeeSched";
                commandSQL = "Insert into " + JurisDbName + ".dbo.FeeSchedule(FeeSchCode,FeeSchDesc,FeeSchActive) Select FeeSchCode,FeeSchDesc,FeeSchActive from " + origDB + ".dbo.FeeSchedule where exists (select matfeesch from " + origDB + ".dbo.matter inner join billto on matbillto=billtosysnbr inner join " + origDB + ".dbo.employee on employee.empsysnbr=billto.billtobillingatty where " + origDB + ".dbo.employee.EmpID in ('" + billAtty + "')) " +
                    " or exists  (Select FeeSchCode,FeeSchDesc,FeeSchActive from " + origDB + ".dbo.FeeSchedule where exists (select clifeesch from " + origDB + ".dbo.client inner join " + origDB + ".dbo.employee on employee.empsysnbr=client.clibillingatty where exists (select empid from " + origDB + ".dbo.employee where EmpID in ('" + billAtty + "'))) ) or " + origDB + ".dbo.FeeSChedule.feeschcode='STDR'";
                _jurisUtility.ExecuteNonQueryCommand(0, commandSQL);
                ...
                errorTable = "ExpSched";
                commandSQL = "Insert into " + JurisDbName + ".dbo.ExpenseSchedule(ExpSchCode,ExpSchDesc) Select ExpSchCode,ExpSchDesc from " + origDB + ".dbo.ExpenseSchedule where  expschcode in (select matexpsch from " + origDB + ".dbo.matter inner join billto on matbillto=billtosysnbr inner join " + origDB + ".dbo.employee on employee.empsysnbr=billto.billtobillingatty where " + origDB + ".dbo.employee.EmpID in ('" + billAtty + "'))" +
                    " or expschcode in (select cliexpsch from " + origDB + ".dbo.client inner join " + origDB + ".dbo.employee on employee.empsysnbr=client.clibillingatty where exists (select empid from " + origDB + ".dbo.employee where EmpID  in ('" + billAtty + "'))) or expschcode='STDR'";
                _jurisUtility.ExecuteNonQueryCommand(0, commandSQL);
                ...
                commandSQL = "Insert into " + JurisDbName + ".dbo.Employee(EmpSysNbr,EmpID,EmpInitials,EmpPassword,EmpValidAsUser,EmpValidAsTkpr,EmpMnuPerm,EmpRptPerm,EmpOnLine,EmpPreferences,EmpPrsTyp,EmpSortSeq,EmpFeeTaxExempt,EmpBudHrsDay,EmpBudTargetRate,EmpNetID,EmpFirstName,EmpMiddleName,EmpLastName,EmpEmail) " +
                    ...
                    " in (select MatSysNbr from " + origDB + ".dbo.matter inner join " + origDB + ".dbo.billto on matbillto=billtosysnbr inner join " + origDB + ".dbo.employee on empsysnbr=billtobillingatty where billtobillingatty in (select EmpSysNbr from " + origDB + ".dbo.employee where empid in ('" + billAtty + "')))))" +
                    " or empsysnbr in (Select clibillingatty from " + origDB + ".dbo.client inner join " + origDB + ".dbo.employee on employee.empsysnbr=client.clibillingatty where EmpID in ('" + billAtty + "'))" +
                    " or empsysnbr in (select billtobillingatty from " + origDB + ".dbo.billto inner join " + origDB + ".dbo.matter on matbillto=billtosysnbr inner join " + origDB + ".dbo.employee on empsysnbr=billtobillingatty where billtobillingatty in (select EmpSysNbr from " + origDB + ".dbo.employee where empid in ('" + billAtty + "')))" +
                    " or empsysnbr in (select morigatty from " + origDB + ".dbo.matorigatty inner join " + origDB + ".dbo.matter on morigmat=matsysnbr inner join " + origDB + ".dbo.employee on empsysnbr=morigatty where EmpID in ('" + billAtty + "'))" +
                    " or empsysnbr in (select corigatty from " + origDB + ".dbo.cliorigatty inner join " + origDB + ".dbo.client on corigcli=clisysnbr inner join " + origDB + ".dbo.employee on empsysnbr=corigatty where EmpID in ('" + billAtty + "'))" +
                    " or empsysnbr in (select MRTEmployeeID from " + origDB + ".dbo.MatterResponsibleTimekeeper inner join " + origDB + ".dbo.matter on MRTMatterID=matsysnbr inner join " + origDB + ".dbo.employee on empsysnbr=mrtemployeeid where EmpID in ('" + billAtty + "'))" +
                    " or empsysnbr in (select CRTEmployeeID from " + origDB + ".dbo.ClientResponsibleTimekeeper inner join " + origDB + ".dbo.client on CRTClientID=clisysnbr inner join " + origDB + ".dbo.employee on empsysnbr=crtemployeeid where EmpID in ('" + billAtty + "'))";
                ...
                commandSQL = "Insert into " + JurisDbName + ".dbo.Client(CliSysnbr,CliCode,CliNickName,CliReportingName,CliSourceOfBusiness,CliPhoneNbr,CliFaxNbr,CliContactName,CliDateOpened,Cliofficecode,CliBillingAtty,CliPracticeClass,CliFeeSch,CliTaskCodeXref,CliExpSch,CliExpCodeXref,CliBillFormat,CliBillAgreeCode,CliFlatFeeIncExp,CliRetainerType,CliExpFreqCode,CliFeeFreqCode,CliBillMonth,CliBillCycle,CliExpThreshold,CliFeeThreshold,CliInterestPcnt,CliInterestDays,CliDiscountOption,CliDiscountPcnt,CliSurchargeOption,CliSurchargePcnt,CliTax1Exempt,CliTax2Exempt,CliTax3Exempt,CliBudgetOption,CliReqPhaseOnTrans,CliReqTaskCdOnTime,CliReqActyCdOnTime,CliReqTaskCdOnExp,CliPrimaryAddr,CliType,CliEditFormat,CliThresholdOption,CliRespAtty,CliBillingField01,CliBillingField02,CliBillingField03,CliBillingField04,CliBillingField05,CliBillingField06,CliBillingField07,CliBillingField08,CliBillingField09,CliBillingField10,CliBillingField11,CliBillingField12,CliBillingField13,CliBillingField14,CliBillingField15,CliBillingField16,CliBillingField17,CliBillingField18,CliBillingField19,CliBillingField20,CliCTerms,CliCStatus,CliCStatus2) " +
                    ...
                    " where " + origDB + ".dbo.Client.clicode in (select clicode from " + origDB + ".dbo.client where CliBillingATty in (select empsysnbr from employee where empid in ('" + billAtty + "'))) or " + origDB + ".dbo.Client.clisysnbr in (select matclinbr from " + origDB + ".dbo.matter inner join billto on matbillto=billtosysnbr inner join " + origDB + ".dbo.employee on employee.empsysnbr=billto.billtobillingatty where " + origDB + ".dbo.employee.EmpID in ('" + billAtty + "'))";
                ...
                commandSQL = "update " + JurisDbName + ".dbo.Client set CliBillingATty = (select top 1 empsysnbr from " + JurisDbName + ".dbo.employee where empid in ('" + billAtty + "')) where CliBillingATty not in (select empsysnbr from " + JurisDbName + ".dbo.employee where empid in ('" + billAtty + "'))";
                _jurisUtility.ExecuteNonQueryCommand(0, commandSQL);
                ...
                commandSQL = "Insert into " + JurisDbName + ".dbo.BillCopy(BilCpyBillTo,BilCpyBilAdr,BilCpyComment,BilCpyNbrOfCopies,BilCpyPrintFormat,BilCpyEmailFormat,BilCpyExportFormat,BilCpyARFormat) " +
                    ...
                    " and " + origDB + ".dbo.BillingAddress.BilAdrNickName=BA.BilAdrNickName and " + origDB + ".dbo.BillingAddress.BilAdrName=BA.BilAdrName and Cli.CliSysnbr=BA.biladrclinbr" +
                    " where " + origDB + ".dbo.billcopy.bilcpybillto in (select matbillto from " + origDB + ".dbo.matter inner join " + origDB + ".dbo.billto on matbillto=billtosysnbr inner join " + origDB + ".dbo.employee on employee.empsysnbr=billto.billtobillingatty where " + origDB + ".dbo.employee.EmpID in ('" + billAtty + "'))";
                _jurisUtility.ExecuteNonQueryCommand(0, commandSQL);
                ...
                commandSQL = "Insert into " + JurisDbName + ".dbo.Matter(MatSysNbr,MatCliNbr,MatBillTo,MatCode,MatNickName,MatReportingName,MatDescription,MatRemarks,MatPhoneNbr,MatFaxNbr,MatContactName,MatDateOpened,MatStatusFlag,MatLockFlag,MatDateClosed,MatOfficeCode,MatPracticeClass,MatFeeSch,MatTaskCodeXref,MatExpSch,MatExpCodeXref,MatQuickAction,MatBillAgreeCode,MatFlatFeeIncExp,MatRetainerType,MatFltFeeOrRetainer,MatExpFreqCode,MatFeeFreqCode,MatBillMonth,MatBillCycle,MatExpThreshold,MatFeeThreshold,MatInterestPcnt,MatInterestDays,MatDiscountOption,MatDiscountPcnt,MatSurchargeOption,MatSurchargePcnt,MatSplitMethod,MatSplitThreshold,MatSplitPriorAmtBld,MatBudgetOption,MatBudgetPhase,MatReqPhaseOnTrans,MatReqTaskCdOnTime,MatReqActyCdOnTime,MatReqTaskCdOnExp,MatTax1Exempt,MatTax2Exempt,MatTax3Exempt,MatDateLastWork,MatDateLastExp,MatDateLastBill,MatDateLastStmt,MatDateLastPaymt,MatLastPaymtAmt,MatARLastBill,MatPaySinceLastBill,MatAdjSinceLastBill,MatPPDBalance,MatVisionAddr,MatThresholdOption,MatType,MatBillingField01,MatBillingField02,MatBillingField03,MatBillingField04,MatBillingField05,MatBillingField06,MatBillingField07,MatBillingField08,MatBillingField09,MatBillingField10,MatBillingField11,MatBillingField12,MatBillingField13,MatBillingField14,MatBillingField15,MatBillingField16,MatBillingField17,MatBillingField18,MatBillingField19,MatBillingField20,MatCTerms,MatCStatus,MatCStatus2) " +
                    ...
                    " inner join " + origDB + ".dbo.employee on employee.empsysnbr=billto.billtobillingatty where " + origDB + ".dbo.employee.EmpID in ('" + billAtty + "')";
